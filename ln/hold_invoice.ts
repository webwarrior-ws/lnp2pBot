...
const createHoldInvoice = async ({ description, amount }: { description: string, amount: number }) => {
  try {
    ...
    if (holdInvoiceCltvDelta === undefined)
      throw new Error("Environment variable HOLD_INVOICE_CLTV_DELTA is not defined");
    const cltv_delta = parseInt(holdInvoiceCltvDelta);
