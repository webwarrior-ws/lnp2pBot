...
export const setComm = async (ctx: MainContext) => {
  try {
    ...
    const maybeGroupName = await validateParams(
      ctx,
      2,
      '\\<_@communityGroupName \\| telegram\\-group\\-id / off_\\>'
    );
    if (maybeGroupName === null || maybeGroupName.length == 0) {
      return;
    }
    const [groupName] = maybeGroupName;
...
export const communityAdmin = async (ctx: CommunityContext) => {
  try {
    const maybeGroup = await validateParams(ctx, 2, '\\<_community_\\>');
    if (maybeGroup === null || maybeGroup.length == 0) return;
...
export const findCommunity = async (ctx: CommunityContext) => {
  try {
    const maybeFiatCode = await validateParams(ctx, 2, '\\<_fiat code_\\>');
    if (maybeFiatCode === null || maybeFiatCode.length == 0) return;
    const [fiatCode] = maybeFiatCode;
