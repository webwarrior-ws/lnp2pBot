...
const dispute = async (ctx: MainContext) => {
  try {
    ...
    const maybeOrderId = await validateParams(ctx, 2, '\\<_order id_\\>');
    if(maybeOrderId == null || maybeOrderId.length == 0) return;
    const [orderId] = maybeOrderId;
